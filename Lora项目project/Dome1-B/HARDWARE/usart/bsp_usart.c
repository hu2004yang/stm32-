...
void USART1_IRQHandler(void)                                //串口1中断服务程序
{
    ...
    {
        Res=USART_ReceiveData(USART1);//接收模块的数据;读取数据寄存器清除RXNE标志
        uint32_t temp = USART1->SR;  // 读取状态寄存器
        temp = USART1->DR;            // 读取数据寄存器，这一步清除IDLE标志
        (void)temp;                   // 避免编译器警告
				buf_uart1.rx_flag=1;
    }
...
void USART2_IRQHandler(void)                            //串口2接收函数
{
    ...
    {
        Res=USART_ReceiveData(USART2);//接收模块的数据;读取数据寄存器清除RXNE标志
        uint32_t temp = USART2->SR;  // 读取状态寄存器
        temp = USART2->DR;            // 读取数据寄存器，这一步清除IDLE标志
        (void)temp;                   // 避免编译器警告
        buf_lora.rx_flag=1;
    }
}
...
void USART3_IRQHandler(void)                                //串口3中断服务程序
{
    ...
    if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)  //接收中断，可以扩展来控制
    {
		...
		if ( Esp8266_rx_length < ( 1024 - 1 ) )
			Esp8266_rx_buf[Esp8266_rx_length++] = Res;
    }
    if(USART_GetITStatus(USART3, USART_IT_IDLE) != RESET)  //模块空闲
    {
        Res=USART_ReceiveData(USART3);//接收模块的数据;读取数据寄存器清除RXNE标志
        uint32_t temp = USART3->SR;  // 读取状态寄存器
        temp = USART3->DR;            // 读取数据寄存器，这一步清除IDLE标志
        (void)temp;                   // 避免编译器警告
				buf_uart3.rx_flag=1;
