...
#define GATTS_TABLE_TAG "gatt"

#define HEART_PROFILE_NUM                         1
#define HEART_PROFILE_APP_IDX                     0
#define ESP_HEART_RATE_APP_ID                     0x55
#define EXAMPLE_DEVICE_NAME                       "smart_lock"
...
extern void App_Communication_RecvData(uint8_t *data,uint16_t len);
    ...
    switch (event) {
        case ESP_GATTS_REG_EVT:
           { esp_ble_gap_set_device_name(EXAMPLE_DEVICE_NAME);
            ...
            break;}
        case ESP_GATTS_READ_EVT: //读事件：手机从锁读数据到这里
            break;
        case ESP_GATTS_WRITE_EVT://写事件：手机向锁写数据到这里
            App_Communication_RecvData(param->write.value,param->write.len);
            break;
